...
- 🗓 **Daily Challenge** — one free seeded run a day with its own leaderboard and streaks. Beyond that, play as many credit-funded custom runs as you've earned.
- 🧭 **Step-by-step setup** — choosing Daily or a custom run opens a guided wizard (formation → draft mode → difficulty → player pool → sign-up → play) instead of a wall of options. The Daily is fully configurable; its board therefore mixes configs (the ×1.1 daily bonus still applies).
